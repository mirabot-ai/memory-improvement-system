# Examples

Sample outputs from the memory mining system.

## Files

- `sample-memory-candidates.md` — Example memory candidate file
- `sample-skill-candidates.md` — Example skill candidate file  
- `sample-review-summary.md` — Example review checklist

## How to use

These examples show what the nightly mining scripts produce. Use them to:

1. Understand the output format
2. Practice the review workflow
3. Calibrate your approval criteria

In actual use, your workspace will accumulate these files in:
- `memory/candidates/`
- `memory/skills/candidates/`
